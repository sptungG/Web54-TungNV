...
const dirPath = "./public/controller/";

const getAllComments = async () => {
  try {
    const jsonComments = await fs.promises.readFile(`${dirPath}comments.json`, "utf-8");
...
const getCommentsByPost = async (postId) => {
  try {
    ...
    const jsonPosts = await fs.promises.readFile(`${dirPath}posts.json`, "utf-8");
...
const createComment = async (postId, dataComment) => {
  try {
    ...
    await fs.promises.writeFile(`${dirPath}comments.json`, JSON.stringify(comments));
    return newComment;
  } catch (err) {
    console.log(err);
    return null;
...
const deleteComment = async (commentId) => {
  try {
    ...
    await fs.promises.writeFile(`${dirPath}comments.json`, JSON.stringify(newComments));
    return true;
  } catch (err) {
    console.log(err);
    return false;
